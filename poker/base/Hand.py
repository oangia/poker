class Hand:
    ...
    def detect(self):
        ...
        self.handType = 0
        self.zitch_point = 0
        self.handType = self.typeDetect()
        self.point = self.calculatePoint()
        return self.handType * 10 + self.point
        
    def calculatePoint(self):
        if self.handType in [Hand.ZITCH, Hand.FLUSH, Hand.STRAIGHT, Hand.STRAIGHT_FLUSH]:
            return self.power/self.MAX_ZITCH_POWER

        if self.handType in [Hand.THREEKIND, Hand.FULLHOUSE, Hand.FOURKIND]:
            return self.cards[2].point

        if self.handType == Hand.ONEPAIR:
    ...
    def typeDetect(self):
        ...
        return straight * Hand.STRAIGHT + flush * Hand.FLUSH - flush * straight
